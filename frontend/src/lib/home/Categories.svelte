<script lang="ts">
  ...
  let categoriesActivePage = 0
  const paginateArray = (pageSize = 10) => {
    var newArr = []
    var maxLength = categoriesArray.length
    var newStart = 0
    for (let i = 0; i < Math.round(maxLength/ 10); i++) {
      newStart = i * pageSize
      newArr.push(categoriesArray.slice(newStart, newStart + pageSize))
    }
    return newArr
  }

  let pageButtonEditable = false
  $: pageInputValue = categoriesActivePage
  ...
  const pageInputHandler = (e: KeyboardEvent) => {
    switch(e.key) {
      case 'Escape':
        pageInputValue = categoriesActivePage + 1
        pageButtonEditable = false
        break;
      case 'Enter':
        categoriesActivePage = pageInputValue - 1
        pageButtonEditable = false
    }
  }
  ...
  imagesArrayData.subscribe((imgs) => {
    let obj:CategoryTableRow  = {}
    imgs.forEach(i => {
      i.category
        .filter(c => !(/\d/g).test(c))
        .forEach(c => {
    ...
    for(let [key, value] of Object.entries(obj)) {
      categories.push({category: key, usage: value})
    }
    categoriesArray = categories.sort((a, b) => b.usage - a.usage)
    pages = paginateArray()
  })
...
{:else}
  <!-- Table -->
  <div class="px-4 py-2">
    <h1>Total Items: {categoriesArray.length}</h1>
    <div class="flex w-full overflow-x-auto">
      <table class="table-hover table">
        ...
        <tbody>
          {#each pages[categoriesActivePage] as i}
    ...
    <div class="pagination mt-4">
      <span class="tooltip tooltip-top" data-tooltip="Previous Page">
        <button disabled={categoriesActivePage === 0} on:click={() => categoriesActivePage -= 1} class="btn">
      ...
      <span class="tooltip tooltip-top" data-tooltip="First Page">
        <button class="btn {categoriesActivePage === 0 ? 'btn-active' : ''}" on:click={() => categoriesActivePage = 0} >{1}</button>
      </span>
      ...
      {:else}
        <button on:click={() => {pageButtonEditable = true}} class="btn { categoriesActivePage > 0 && categoriesActivePage < pages.length - 1 ? 'btn-active' : ''}" on:click={() => categoriesActivePage = 0} >{categoriesActivePage > 0 && categoriesActivePage < pages.length - 1 ? categoriesActivePage + 1: '...'}</button>
      {/if}
      <span class="tooltip tooltip-top" data-tooltip="Last Page">
        <button class="btn {categoriesActivePage === pages.length - 1 ? 'btn-active' : ''}" on:click={() => categoriesActivePage = pages.length - 1} >{pages.length}</button>
      </span>
      <span class="tooltip tooltip-top" data-tooltip="Next Page">
        <button disabled={categoriesActivePage + 1 === pages.length} on:click={() => categoriesActivePage += 1} class="btn">
          <svg width="18" height="18" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
