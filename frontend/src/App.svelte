<script lang="ts">
  ...
  import type {Image, src} from '../wailsjs/go/models'
    ...
    <div class="content">
      ...
      {:else}
        <div class="tabs p-3 w-full justify-center bg-slate-3">
          ...
          <button on:click={() => activeTab = 1} class="{activeTab === 1 ? activeTabClasses : '' } transition duration-200 tab px-6">Categories</button>
          <button on:click={() => activeTab = 2} class="{activeTab === 2 ? activeTabClasses : '' } transition duration-200 tab px-6">Tags</button>
        </div>
